...
Around 2005, the American scientist Louis Scheffer provided a cryptoanalysis of Malbolge, uncovering several weaknesses in the design---like cycles in the en- and decryption substition tables---which make it possible to avoid some of Malbolge's complications \cite{schefferintroduction}. He described how to store and load values, to do simple arithmatic, and to avoid self-modification in a systematical manner. He then used these ideas to describe how a Brainfuck-to-Malbolge compiler could work \cite{schefferwriting}, which would be a formal proof of Turing completeness (again, ignorig memory limitations). Finally, Scheffer gave some ideas to make Malbolge \emph{even harder}, like improving the substition tables, making the crazy operation less useful, or modifying instructions \emph{before} they are executed, not after.
